...
class AsyncRedisDB(aioredis.Redis):
    ...
    def _load_lua_script(self):
        """ 加载 lua 脚本 """
        self._zset_set_score_script = self.register_script("""
            local key = KEYS[1]
        ...
        self._zset_increase_score_script = self.register_script("""
            local key = KEYS[1]
            local min_score = ARGV[1]
            local max_score = ARGV[2]
        ...
        self._zser_del_score_script = self.register_script("""
            local key = KEYS[1]
            local min_score = ARGV[1]
            local max_score = ARGV[2]
            local num = ARGV[3]
    ...
    async def zset_set_by_score(self, name: str, min: int, max: int, score: int, num: int = '') -> list:
        """ 获取指定区间数据并修改分数为指定值 """
        return await self._zset_set_score_script(keys=[name], args=[min, max, score, num])

    async def zset_increase_by_score(self, name: str, min: int, max: int, increment: int, num: int = '') -> list:
        """ 获取指定区间数据并修改分数加上增量 """
        return await self._zset_increase_score_script(keys=[name], args=[min, max, increment, num])

    async def zset_del_by_score(self, name: str, min: int, max: int, num: int = '') -> list:
        """ 获取指定区间数据并删除 """
        return await self._zser_del_score_script(keys=[name], args=[min, max, num])
